...
data %>% 
  ...
  filter(MAGNITUDE>46) -> a

data %>% 
  filter(EVENT_TYPE %in% c("Thunderstorm Wind")) %>% 
  filter(!is.na(MAGNITUDE)) %>% 
  filter(MAGNITUDE>0) -> b

(nrow(b) - nrow(a))/nrow(b)

to_pres <- setdiff(b$EVENT_ID, a$EVENT_ID)

data %>% 
  filter(EVENT_ID%in%to_pres) -> filtered_storms

saveRDS(filtered_storms,file="data/filtered_storms.rds")
