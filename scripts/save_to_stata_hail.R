...
data %<>% 
  mutate(
    lag1 = ifelse(Year==1991, as.numeric(NA), dplyr::lag(S3,1)),
    lag2 = ifelse(Year==1992, as.numeric(NA), dplyr::lag(S3,2)),
    lag3 = ifelse(Year==1993, as.numeric(NA), dplyr::lag(S3,3)),
    lag4 = ifelse(Year==1994, as.numeric(NA), dplyr::lag(S3,4)),
    lag5 = ifelse(Year==1995, as.numeric(NA), dplyr::lag(S3,5)),
    lag6 = ifelse(Year==1996, as.numeric(NA), dplyr::lag(S3,6)),
    lag7 = ifelse(Year==1997, as.numeric(NA), dplyr::lag(S3,7)),
    lag8 = ifelse(Year==1998, as.numeric(NA), dplyr::lag(S3,8)),
    lag9 = ifelse(Year==1999, as.numeric(NA), dplyr::lag(S3,9)),
    lag10 = ifelse(Year==2000, as.numeric(NA), dplyr::lag(S3,10))
  ) 

write.csv(data, "data/panel_hail_csv.csv", row.names = FALSE)
haven::write_dta(data, "stata_rob/panel_hail_dta.dta")